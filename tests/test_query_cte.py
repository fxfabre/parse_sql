...
b2c_demandes = OrderedDict(
    ...
    join_prospect_intentionistes___1={"join": [], "select": {}, "tables": {}},
    join_prospect_intentionistes___2={"join": [], "select": {}, "tables": {}},
    __query__={
        "join": [
...
query_join_alias_2 = OrderedDict(
    __query__={
        "join": [(["table1", "c1"], ["table2", "c2"])],
        "select": {"*": "*", "col_1": "col_1", "name_2": "col_2"},
        "tables": {"alias_T1": "dataset_1.table", "table2": "dataset_2.table2"},
...
appel_code_cloture = OrderedDict(
    __query__={"join": [], "select": {}, "tables": {}},
)
...
class TestEachSql(TestCase):
    @parameterized.expand(
        [
            (query_cte, "query_cte.sql"),
            (query_groupby, "query_groupby.sql"),
            (query_groupby_2, "query_groupby_2.sql"),
            (query_join, "query_join.sql"),
            (query_join_2, "query_join_2.sql"),
            (query_join_alias, "query_join_alias.sql"),
            (query_join_alias_2, "query_join_alias_2.sql"),
            (query_where, "query_where.sql"),
            # (b2c_demandes, "b2c_demandes.sql"),     # UNION ALL
            # (appel_code_cloture, "appel_code_cloture.sql"),   # , UNNEST(...) vs CROSS JOIN
        ]
    )
    def test_sql_parsing(self, expected, file_name):
        file_path = Path("tests/sql_test_files") / file_name
        parsing_by_cte = parse_sql_file(file_path.read_text())
        self.assertEqual(parsing_by_cte, expected)
